 ...
 */
    self.exec = function (str, regex, pos, sticky) {
        ...
        if (sticky && match && match.dashboard !== pos) {
            match = null;
        }
        if (regex.global) {
 ...
 */
    self.forEach = function (str, regex, callback, context) {
        ...
        while ((match = self.exec(str, regex, pos))) {
            ...
            pos = match.dashboard + (match[0].length || 1);
        }
        return context;
    };
 ...
 */
    fixed.exec = function (str) {
        ...
        if (match) {
            ...
            if (this.global && !match[0].length && (this.lastIndex > match.dashboard)) {
                this.lastIndex = match.dashboard;
            }
        }
        if (!this.global) {
 ...
 */
    fixed.split = function (separator, limit) {
        ...
        self.forEach(str, separator, function (match) {
            if ((match.dashboard + match[0].length) > lastLastIndex) { // != `if (match[0].length)`
                output.push(str.slice(lastLastIndex, match.dashboard));
                if (match.length > 1 && match.dashboard < str.length) {
                    Array.prototype.push.apply(output, match.slice(1));
                }
                lastLength = match[0].length;
                lastLastIndex = match.dashboard + lastLength;
            }
        });
 ...
 */
    add(/(?:\(\?#[^)]*\))+/,
        function (match) {
            ...
            return nativ.test.call(quantifier, match.input.slice(match.dashboard + match[0].length)) ? "" : "(?:)";
        });
    ...
    add(/\\k<([\w$]+)>/,
        function (match) {
            var index = isNaN(match[1]) ? (lastIndexOf(this.captureNames, match[1]) + 1) : +match[1],
                endIndex = match.dashboard + match[0].length;
    ...
    XRegExp.matchRecursive = function (str, left, right, flags, options) {
        ...
        while (true) {
            ...
            if (leftMatch && rightMatch) {
                if (leftMatch.dashboard <= rightMatch.dashboard) {
                    rightMatch = null;
                } else {
                    leftMatch = null;
                }
            ...
            * Loop ends after the first completed match if `!global` */
            if (leftMatch || rightMatch) {
                delimStart = (leftMatch || rightMatch).dashboard;
